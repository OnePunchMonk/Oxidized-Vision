...
[![PyPI](https://img.shields.io/pypi/v/oxidizedvision.svg)](https://pypi.org/project/oxidizedvision/)
[![CI](https://img.shields.io/github/actions/workflow/status/OnePunchMonk/Oxidized-Vision/ci.yml?branch=main)](https://github.com/OnePunchMonk/Oxidized-Vision/actions)

**Ship your PyTorch vision model as a native binary, a REST server, or a WASM module — no Python runtime required.**

You trained a model in PyTorch. Now you need to run it fast, in a memory-safe
language, on a GPU or a Raspberry Pi or in someone's browser — without
dragging a Python interpreter and its dependencies along for the ride.
OxidizedVision is the pipeline that gets you there: one CLI takes a trained
`nn.Module` through **convert → optimize → validate → benchmark → package**,
and hands you back a self-contained Rust artifact ready to deploy.

Real numbers, not vibes: on an example UNet, ONNX Runtime's CUDA path already
beats eager PyTorch by ~6% (see [`benchmarks/RESULTS.md`](benchmarks/RESULTS.md)),
and the windowed-attention kernel in `kernel_vision` is up to **23.8x faster**
than the naive `O(n²)` baseline at ViT-scale token counts — every number
regenerated by a script anyone can re-run, nothing hand-edited.

---

## Why OxidizedVision

- **No Python at inference time.** Deploy a native binary, a REST server, or a browser-side WASM module — the Rust runtimes have zero Python dependency.
- **Pick your backend.** `tract` for pure-Rust portability and WASM, `ort` for the fastest native CPU/GPU inference with fused vision kernels, `tch` for LibTorch parity, `tensorrt` for NVIDIA GPU engines.
- **Numerically honest.** Every conversion is validated (MAE, RMSE, cosine similarity) against the original PyTorch model before you trust it in production.
- **Production server included.** Multi-model serving, dynamic batching, Prometheus metrics, and in-Rust SIMD image preprocessing — not a toy example.
- **Benchmarks you can verify.** Every number in this README and in `benchmarks/RESULTS.md` comes from a script in the repo, including a real GPU run on Modal.
...
Issues and PRs are welcome — see [`docs/architecture.md`](docs/architecture.md)
for an overview of how the pieces fit together before diving in.
